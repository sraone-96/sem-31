...
def kernelize(X, Y):
    ...
    ===========================================================================
    """
    KernelX=np.zeros((len(X),3))
    for i in range(len(X)):
		KernelX[i,2]=np.sqrt(X[i,0]*X[i,0]+X[i,1]*X[i,1])
    KernelX[:,0:2]=X[:,0:2] 
    # YOUR CODE HERE
...
def perceptron(X, Y):
    ...
    ==========================================================================
    """
    #sklearn.linear_model.Perceptron(penalty=None, alpha=0.0001, fit_intercept=True, max_iter=None, tol=None, shuffle=True, n_iter=None)
    # YOUR CODE GOES HERE
