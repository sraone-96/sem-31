...
# Fit the model
model.fit(xnew, y,epochs=8, batch_size=128)
...
print (accuracy_score(ytemp,final))
from keras.models import Model
inter_layer_model=Model(inputs=model.input,outputs=model.layers[13].output)
inter_output_train=inter_layer_model.predict(xnew)
inter_output_test=inter_layer_model.predict(xt)
from sklearn import svm
clf = svm.SVC(C=10)
yf=np.zeros(len(y))
for i in range(len(y)):
    index=np.where(y[i]==y[i].max())
    yf[i]=index[0]

clf.fit(inter_output_train,yf)
predictions=clf.predict(inter_output_test)
# print predictions

print (accuracy_score(ytemp,predictions))